...
import { FileText, ClipboardCheck, TrendingUp, Users, ArrowRight, Eye, RefreshCw, Calendar } from "lucide-react";
...
export default function Dashboard({
  ...
  return (
    <div id="smei-dashboard" className="p-6 md:p-10 space-y-8 max-w-7xl mx-auto">
      {/* Welcome Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-neutral-900 p-6 rounded-2xl border border-gray-100 dark:border-neutral-800 shadow-sm">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white tracking-tight font-display">
            SMEI Procurement Dashboard
          </h2>
      ...
      <div className="bg-gradient-to-r from-smei-darkred to-smei-crimson text-white rounded-2xl p-6 shadow-lg shadow-red-950/15 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="space-y-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Monthly Trend Chart */}
        <div className="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-gray-200 dark:border-neutral-800 shadow-sm lg:col-span-2 flex flex-col justify-between h-[400px]">
        ...
        <div className="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-gray-200 dark:border-neutral-800 shadow-sm flex flex-col justify-between overflow-hidden h-[400px]">
            ...
            {topSuppliersData.length > 0 && topSuppliersData[0].value > 0 ? (
              <ResponsiveContainer width="100%" height={Math.max(280, topSuppliersData.length * 40)}>
                <BarChart data={topSuppliersData} layout="vertical" margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="#f0f0f0" className="dark:stroke-neutral-800" />
              ...
              <div className="text-center text-xs text-gray-400 font-sans p-6 border-2 border-dashed border-gray-100 dark:border-neutral-800 rounded-xl h-full flex flex-col items-center justify-center">
      ...
      <div className="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-gray-200 dark:border-neutral-800 shadow-sm space-y-4">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-lg font-bold text-gray-800 dark:text-white font-display">Recent Activity & Transactions</h3>
            <p className="text-xs text-gray-500 dark:text-neutral-400 mt-0.5">Most recently created or updated purchase order records</p>
          </div>
          <button
            onClick={onNavigateToPOList}
        ...
        <div className="overflow-x-auto">
          <table id="smei-recent-transactions-table" className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-neutral-800/50 text-gray-500 dark:text-neutral-400 text-xs uppercase tracking-wider font-semibold border-b border-gray-100 dark:border-neutral-800">
                <th className="py-3.5 px-4 font-display">PO Number</th>
                <th className="py-3.5 px-4 font-display">Supplier</th>
                <th className="py-3.5 px-4 font-display">Purpose / Subject</th>
                <th className="py-3.5 px-4 font-display">Date Issued</th>
                <th className="py-3.5 px-4 font-display text-right">Total (PHP)</th>
                <th className="py-3.5 px-4 font-display">Workflow Status</th>
                ...
                return (
                  <tr
                    ...
                    className={`hover:bg-gray-50 dark:hover:bg-neutral-800/50 transition-colors cursor-pointer ${
                      index % 2 === 1 ? "bg-gray-50/30 dark:bg-neutral-800/20" : "bg-white dark:bg-transparent"
                    }`}
                    title="Double-click to View/Edit PO"
                  >
                    <td className="py-3.5 px-4 font-mono font-bold text-smei-darkred dark:text-rose-400">{po.poNumber}</td>
                    <td className="py-3.5 px-4 font-medium text-gray-800 dark:text-white truncate max-w-[150px]">{po.supplierName}</td>
                    <td className="py-3.5 px-4 text-gray-500 dark:text-neutral-400 truncate max-w-[200px]">{po.purpose}</td>
                    <td className="py-3.5 px-4 text-gray-500 dark:text-neutral-400">{po.poDate}</td>
                    <td className="py-3.5 px-4 text-right font-mono font-bold text-gray-800 dark:text-white">{formatPHP(po.totalAmount)}</td>
                    <td className="py-3.5 px-4">
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${statusColors[po.status] || "bg-gray-100 dark:bg-neutral-800"}`}>
                        {po.status}
                      </span>
                    </td>
