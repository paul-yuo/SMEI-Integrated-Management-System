...
export default function RoleManagement() {
  ...
  useEffect(() => {
    const defaultRules = [
      ...
      { id: "po_status_change", moduleName: "Purchase Order", actionName: "Access Purchase Orders", pinCode: "1111", isEnabled: false },
      { id: "pis_access", moduleName: "Payment Instruction Slip", actionName: "Access Payment Instruction Slip", pinCode: "2222", isEnabled: false },
      { id: "rfs_access", moduleName: "Request For Supply", actionName: "Access Request For Supply", pinCode: "3333", isEnabled: false },
      { id: "canvass_access", moduleName: "Canvass Sheet", actionName: "Access Canvass Sheets", pinCode: "4444", isEnabled: false }
    ];

    const saved = localStorage.getItem("smei_module_pins");
    if (saved) {
      try {
        let loaded: PinProtectionRule[] = JSON.parse(saved);
        // Ensure all 4 approval gates exist
        let changed = false;
        defaultRules.slice(0, 4).forEach((defRule) => {
          if (!loaded.some(r => r.id === defRule.id)) {
            loaded.unshift(defRule);
            changed = true;
          }
        });
        if (changed) {
